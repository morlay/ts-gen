...
export class Type extends Stringable implements IType {
  ...
  static of(type: any) {
    return new Type({ name: String(type) });
  }
  ...
  static objectOf(...props: Identifier[]) {
    return Type.of(stringifyMembers(";", ...props.map((i: Identifier) => {
      return `${i.typed(Type.of(indentAllExpectFirst(String(i.type || Type.any())))).reset("types").reset("value")}`;
    })));
  }

  static enumOf(...keys: Identifier[]) {
    return Type.of(stringifyMembers(",", ...keys.map((i: Identifier) => {
      return `${Identifier
        .of(i.name)
        .valueOf(i.value)
        }`;
...
export class Identifier extends Stringable implements IIdentifier {
  ...
  toString() {
    ...
    if (this.indexer) {
      result = `[${Identifier.of(this.indexer.name).typed(this.indexer.type || Type.string())}]`;
    }
  ...
  extendsWith(...identifiers: Identifier[]) {
    return this.set("extends", identifiers.map((i) => Identifier.of(i.name).generics(...(i.types || []))));
  }
...
export class Value extends Stringable {
  ...
  static objectOf(...props: Identifier[]) {
    return new Value(stringifyMembers(",", ...props.map((i: IIdentifier) => {
