...
const resolveRef = (imports: IImports) =>
  (value: any): any => {
    if (!lodash.isArray(value) && lodash.isObject(value)) {
      ...
      if (lodash.isString(ref)) {
        ...
        return lodash.cloneDeepWith(lodash.get(imports[importRef], keyPathArr), resolveRef({
          "#": imports[importRef],
        }));
      }
    }
...
export const isObjectType = (schema: IJSONSchema): boolean => schema.type === "object" || hasProps(schema, [
  "properties",
...
export const isArrayType = (schema: IJSONSchema): boolean => schema.type === "array" || hasProps(schema, [
  "items",
  "additionalItems",
  "maxItems",
  "minItems",
...
export const isNumberType = (schema: IJSONSchema): boolean => schema.type === "number" || schema.type === "integer" || hasProps(schema, [
  "maximum",
  "minimum",
]);

export const isStringType = (schema: IJSONSchema): boolean => schema.type === "string" || hasProps(schema, [
  "maxLength",
  "minLength",
]);
...
export const toUpperCamelCase = (s: string): string => {
  return lodash.reduce(lodash.words(s), (result, word) => {
    return result + toCamel(word);
  }, "");
};

export const toLowerCamelCase = (s: string): string => {
  return lodash.reduce(lodash.words(s), (result, word, idx) => {
